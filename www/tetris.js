...
var YOUR_API_KEY = "PiBRJHzWCYY89EOVy83aULDo8WYxWZLq";
var YOUR_PLACEMENT_LOCATION = "inapp";


document.addEventListener('onMediaFinished', function (e) {
    if (e.viewCompleted) {
...
document.addEventListener('deviceready', function () {
    ...
    AppTracker.loadModuleToCache(YOUR_PLACEMENT_LOCATION);
}, false);
...
//        AppTracker.startSession("YOUR_APP_API_KEY");
...
function addScore(n) {
    score = score + n;
    var highScore = parseInt($("#Highestscore").html());

    if (highScore < score) { $("#Highestscore").html(score); }
    if (score > 0 && score <= 250) {
        $("#spanForLevel").html('1');
        speed.decrement = 0.005;
        step = Math.max(speed.min, speed.start - (speed.decrement * rows));
    }
    else if (score > 250 && score <= 450) {
        $("#spanForLevel").html('2');
        speed.decrement = 0.05;
        step = Math.max(speed.min, speed.start - (speed.decrement * rows));
    }
...
function drawNext() {
    if (invalid.next) {
        var padding = 0;//(nu - next.type.size) / 2; // half-arsed attempt at centering next piece display
        uctx.save();
